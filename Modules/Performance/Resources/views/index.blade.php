    ...
    <div class="card card-custom">
        <div class="card-header flex-wrap py-5">
            <div class="card-title">
                <h3 class="card-label">Performance List
                    <span class="d-block text-muted pt-2 font-size-sm">Available Performance List</span></h3>
            </div>
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{route('performance.create')}}" class="btn btn-primary font-weight-bolder">
            ...
            <table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
                ...
                @foreach($performances as $performance)
                    <tr>
                        ...
                        <td>{{$performance->heading}}</td>
                        <td>{{$performance->title}}</td>
                        <td><img width="45px;" height="25px;"  src="{{$performance->getMedia('images')->last()->getFullUrl()}}"></td>
                        <td>{!! Str::limit($performance->description,20) !!}</td>
                        <td nowrap="nowrap">
                            ...
                            <form method="POST" action="{{route('performance.destroy',$performance->id)}}">
                                @method('DELETE')
                                @csrf
                                <button type="submit">
