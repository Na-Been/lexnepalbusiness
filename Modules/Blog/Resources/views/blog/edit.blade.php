...
@section('content')
    <div class="card card-custom">
        ...
        <!--begin::Form-->
        <form class="form" method="POST" action="{{route('blog.update',$blog->id)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="form-group">
                    <label>Title</label><span class="text-danger">*</span>
                    <input type="text" name="name" value="{{$blog->name}}" class="form-control form-control-solid"
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="added_by"  value="{{$blog->added_by}}" class="form-control form-control-solid" placeholder="Example input"/>
                </div>
                @error('added_by')
                <div class="text-danger">{{$message}}</div>
                @enderror
                <div class="from-group mt-5">
                    ...
                    <div class="col-lg-9 col-xl-6">
                        <div class="image-input image-input-empty image-input-outline" id="kt_image_5"
                             style="@if($blog->image)
                                 background-image: url({{$blog->getMedia('images')->last()->getFullUrl()}})">
                        @else
