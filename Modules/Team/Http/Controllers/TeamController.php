...
class TeamController extends Controller
{
    ...
    public function store(TeamRequest $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                $validateData['image'] = $team->addMediaFromRequest('image')->toMediaCollection('images');
                $team->update($validateData);
            }
            return redirect()->route('team.index')->with('success', 'Team Created Successfully');
        } catch (Exception $e) {
            return back()->with('failed', 'Team Cannot Be Create');
        }
    ...
    public function destroy(Team $team)
    {
        try {
            ...
            DB::rollBack();
            return back()->with('failed', 'Cannot Delete Team Something Went Wrong');
        }
    }
}
